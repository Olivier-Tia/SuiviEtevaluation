        ...
        If (Ref = "C07") Or (Ref = "C08") Then
            ...
            If Ref = "C07" Then 'Charger les données par Direction et Employé
                ...
                For Each rw As DataRow In dtDivision.Rows
                    ...
                    query = "SELECT SUM(QteNature*PUNature) As Total 
                        FROM t_besoinpartition 
                        WHERE CodePartition IN
                        (
                            SELECT DISTINCT(O.CodePartition) 
                            FROM 
                                t_operateurpartition O, t_partition P, t_grh_travailler T, t_fonction F, t_service S 
                            WHERE 
                                PosteActu='O' AND P.CodePartition=O.CodePartition AND O.TitreOpPart='Responsable' AND T.EMP_ID=O.EMP_ID AND
                                F.RefFonction=T.CodeService AND S.CodeService=F.CodeService AND S.RefDecoupAdmin='" & rw("RefDecoupAdmin") & "'
                            AND P.DateDebutPartition>='" & dateconvert(dtd.Text) & "' AND 
                            P.DateFinPartition<='" & dateconvert(dtf.Text) & "')"
                    ...
                    query = "SELECT * 
                        FROM t_service S 
                        WHERE CodeService IN
                        (
                            SELECT DISTINCT(S.CodeService) 
                            FROM t_service S, t_operateurpartition O, t_partition P, t_grh_travailler T, t_fonction F 
                            WHERE 
                                PosteActu='O' AND P.CodePartition=O.CodePartition AND O.TitreOpPart='Responsable' AND 
                                F.RefFonction=T.CodeService AND S.CodeService=F.CodeService AND P.DateDebutPartition>='" & dateconvert(dtd.Text) & "' AND 
                                P.DateFinPartition<='" & dateconvert(dtf.Text) & "' AND S.RefDecoupAdmin='" & rw("RefDecoupAdmin") & "' AND S.CodeProjet='" & ProjetEnCours & "' 
                        ) 
                        ORDER BY CodeServiceSup ASC"
                    ...
                    For Each rwServcie As DataRow In dtService.Rows
                        ...
                        query = "SELECT SUM(QteNature*PUNature) As Total 
                            ...
                            (
                                SELECT DISTINCT(O.CodePartition) 
                                FROM t_operateurpartition O, t_partition P, t_grh_travailler T, t_fonction F, t_service S
                                WHERE 
                                    PosteActu='O' AND P.CodePartition=O.CodePartition AND T.EMP_ID=O.EMP_ID AND O.TitreOpPart='Responsable' AND 
                                    F.RefFonction=T.CodeService AND S.CodeService=F.CodeService AND P.DateDebutPartition>='" & dateconvert(dtd.Text) & "' AND 
                                    P.DateFinPartition<='" & dateconvert(dtf.Text) & "' AND S.CodeService='" & rwServcie("CodeService") & "' AND S.CodeProjet='" & ProjetEnCours & "'
                            )"
                        ...
                        query = "SELECT * 
                            ...
                            (
                                ...
                                FROM t_operateurpartition O, t_partition P, t_grh_travailler T, t_fonction F 
                                WHERE 
                                    PosteActu='O' AND P.CodePartition=O.CodePartition AND T.EMP_ID=O.EMP_ID AND 
                                    F.RefFonction=T.CodeService AND O.TitreOpPart='Responsable' AND 
                                    P.DateDebutPartition>='" & dateconvert(dtd.Text) & "' AND 
                                    P.DateFinPartition<='" & dateconvert(dtf.Text) & "' AND 
                                    F.CodeService='" & rwServcie("CodeService") & "'
                            )"
                        ...
                        For Each rwEmploye As DataRow In dtEmploye.Rows
                            ...
                            query = "SELECT SUM(QteNature*PUNature) As Total 
                                ...
                                (
                                    ...
                                    FROM t_operateurpartition O, t_partition P, t_grh_travailler T, t_fonction F 
                                    WHERE 
                                        PosteActu='O' AND P.CodePartition=O.CodePartition AND F.RefFonction=T.CodeService AND
                                        O.EMP_ID='" & rwEmploye("EMP_ID") & "' AND P.DateDebutPartition>='" & dateconvert(dtd.Text) & "' AND 
                                        P.DateFinPartition<='" & dateconvert(dtf.Text) & "' AND O.TitreOpPart='Responsable' AND 
                                        F.CodeService='" & rwServcie("CodeService") & "'
                                )"
                ...
                For Each rw As DataRow In dtDivision.Rows
                    ...
                    query = "SELECT SUM(QteNature*PUNature) As Total 
                        ...
                        (
                            ...
                            WHERE 
                                PosteActu='O' AND P.CodePartition=O.CodePartition AND T.EMP_ID=O.EMP_ID AND
                                F.RefFonction=T.CodeService AND S.CodeService=F.CodeService AND S.RefDecoupAdmin='" & rw("RefDecoupAdmin") & "'
                            AND P.DateDebutPartition>='" & dateconvert(dtd.Text) & "' AND O.TitreOpPart='Responsable' AND 
                            P.DateFinPartition<='" & dateconvert(dtf.Text) & "')"
                    MontantDivision = Val(ExecuteScallar(query))
                    If MontantDivision > 0 Then
                        Dim CodeSup As Decimal = Val(rw("RefDecoupSup"))
                    ...
                    For Each rwServcie As DataRow In dtService.Rows
                        ...
                        ElseIf NiveauEdition = "Composante" Then
                            query = "SELECT DISTINCT(MID(LibelleCourt,1,1)) As LibelleCourt,LibellePartition
                            FROM t_partition 
                            WHERE CodePartition IN
                            (
                                SELECT DISTINCT(O.CodePartition) 
                        ...
                        For Each rwPartition As DataRow In dtPartition.Rows
                            ...
                            If NiveauEdition = "Activité" Then
                                query = "SELECT SUM(QteNature*PUNature) As Total 
                                FROM t_besoinpartition 
                                WHERE CodePartition='" & rwPartition("CodePartition") & "'"
                            ElseIf NiveauEdition = "Composante" Or NiveauEdition = "Sous Composante" Then
                                ...
                                (
                                    ...
                                    FROM t_operateurpartition O, t_partition P, t_grh_travailler T, t_fonction F 
                                    WHERE 
                                        PosteActu='O' AND P.CodePartition=O.CodePartition AND F.RefFonction=T.CodeService AND O.EMP_ID=T.EMP_ID AND
                                        P.LibelleCourt LIKE '" & rwPartition("LibelleCourt") & "%' AND P.DateDebutPartition>='" & dateconvert(dtd.Text) & "' AND 
                                        P.DateFinPartition<='" & dateconvert(dtf.Text) & "' AND O.TitreOpPart='Responsable' AND 
                                        F.CodeService='" & rwServcie("CodeService") & "'
                                )"
                            End If
