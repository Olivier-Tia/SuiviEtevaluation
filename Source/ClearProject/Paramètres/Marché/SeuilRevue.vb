...
Public Class SeuilRevue
    ...
    Private Sub RemplirSeuil()
        ...
        For Each rw As DataRow In dt.Rows
            ...
            query = "select * from T_Seuil where CodeProcAO='" & CodeAO & "' order by MontantPlanche"
    ...
    Private Sub BtSupprimer_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtSupprimer.Click
        ...
        If (CodeSeuil <> 0) Then
            Dim RepoSeuil As MsgBoxResult
            RepoSeuil = MsgBox("Voulez-vous supprimer définitivement la ligne" & vbNewLine & Mid(SeuilGrid.Rows.Item(LigneSupp).Cells(0).Value.ToString, 20) & " " & SeuilGrid.Rows.Item(LigneSupp).Cells(1).Value & " " & SeuilGrid.Rows.Item(LigneSupp).Cells(2).Value & " ?", MsgBoxStyle.YesNo)
            If (RepoSeuil = MsgBoxResult.Yes) Then

               query= "DELETE from T_Seuil where CodeSeuil='" & CodeSeuil & "'"
                ExecuteNonQuery(query)

                RemplirSeuil()
            End If
        Else
            MsgBox("Cette ligne ne peut être supprimée!", MsgBoxStyle.Exclamation)
        End If
