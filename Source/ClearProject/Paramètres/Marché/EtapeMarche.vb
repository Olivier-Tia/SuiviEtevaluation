...
Public Class EtapeMarche
    ...
    Private Sub ChargerEtape(IdMethode As String)
        ...
        query = "select * from T_EtapeMarche where CodeProjet='" & ProjetEnCours & "' and TypeMarche='" & CmbTypeMarche.Text & "' AND CodeProcAO='" & IdMethode & "' order by NumeroOrdre"
        Dim dt As DataTable = ExcecuteSelectQuery(query)
        ...
        For Each rw As DataRow In dt.Rows
            ...
            drS(1) = rw("RefEtape").ToString
            drS(2) = rw("NumeroOrdre").ToString
    ...
    Private Sub GridEtape_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles GridEtape.DoubleClick
        ...
        If (ViewEtape.RowCount > 0) Then
            ...
            GbNewEtape.Visible = True
            RefEtape.Text = DrX("Ref").ToString
    ...
    End Sub
    Private Sub UpdateNumeroOrdre(TypeMarche As String, CodeMethode As String, Numero As Integer, Optional OldNumero As Integer = -1, Optional Signe As String = "+")
        If Signe = "+" Then
            If OldNumero = -1 Then
                query = "UPDATE t_etapemarche SET NumeroOrdre=NumeroOrdre" & Signe & "1 WHERE CodeProjet='" & ProjetEnCours & "' AND TypeMarche='" & EnleverApost(TypeMarche) & "' AND CodeProcAO='" & CodeMethode & "' AND NumeroOrdre>='" & Numero & "'"
            Else
                If Numero < OldNumero Then
                    query = "UPDATE t_etapemarche SET NumeroOrdre=NumeroOrdre" & Signe & "1 WHERE CodeProjet='" & ProjetEnCours & "' AND TypeMarche='" & EnleverApost(TypeMarche) & "' AND CodeProcAO='" & CodeMethode & "' AND NumeroOrdre<'" & OldNumero & "' AND NumeroOrdre>='" & Numero & "'"
                ElseIf Numero >= OldNumero Then
                    query = "UPDATE t_etapemarche SET NumeroOrdre=NumeroOrdre-1 WHERE CodeProjet='" & ProjetEnCours & "' AND TypeMarche='" & EnleverApost(TypeMarche) & "' AND CodeProcAO='" & CodeMethode & "' AND NumeroOrdre<='" & Numero & "' AND NumeroOrdre>'" & OldNumero & "'"
                End If
            End If
        Else
            query = "UPDATE t_etapemarche SET NumeroOrdre=NumeroOrdre" & Signe & "1 WHERE CodeProjet='" & ProjetEnCours & "' AND TypeMarche='" & EnleverApost(TypeMarche) & "' AND CodeProcAO='" & CodeMethode & "' AND NumeroOrdre>'" & OldNumero & "'"
        End If
    ...
    Private Sub BtEnreg_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtEnreg.Click
        ...
        If (RefEtape.Text = "") Then   'Nouvel enregistrement

            'Verifier que la ligne n'existe pas deja ******************************************
            query = "select * from T_EtapeMarche where CodeProjet='" & ProjetEnCours & "' and TitreEtape='" & EnleverApost(txtIntitule.Text) & "' and TypeMarche='" & CmbTypeMarche.Text & "' AND CodeProcAO='" & CodeMethode(cmbMethode.SelectedIndex) & "'"
            ...
            query = "INSERT INTO T_EtapeMarche VALUES(NULL,'" & ProjetEnCours & "','" & cmbNumOrdre.Text & "','" & EnleverApost(CmbTypeMarche.Text) & "','" & EnleverApost(txtIntitule.Text.Trim()) & "','" & IIf(RdPriori.Checked, "OUI", "NON") & "','" & IIf(RdPosteriori.Checked, "OUI", "NON") & "','NON','NON','NON','NON','NON','NON','NON','" & CodeMethode(cmbMethode.SelectedIndex) & "','" & IIf(rdDelai.Checked, txtDelaiValue.Value & " " & cmbDelaiMesure.Text, "DAO") & "')"
            ExecuteNonQuery(query)
        ...
        Else ' Modification d'un enregistrement
            ...
            query = "select * from T_PlanMarche where RefEtape='" & RefEtape.Text & "'"
            Dim dt As DataTable = ExcecuteSelectQuery(query)
            If dt.Rows.Count > 0 Then
                query = "UPDATE t_etapemarche SET TitreEtape='" & EnleverApost(txtIntitule.Text.Trim()) & "' WHERE RefEtape='" & RefEtape.Text & "'"
                ExecuteNonQuery(query)
                SuccesMsg("Cette étape est déjà en cours d'utilisation." & vbNewLine & "Seul l'intitulé a été modifié.")
            Else
                query = "UPDATE t_etapemarche SET NumeroOrdre='" & cmbNumOrdre.Text & "',TitreEtape='" & EnleverApost(txtIntitule.Text.Trim()) & "',Priori='" & IIf(RdPriori.Checked, "OUI", "NON") & "',Posteriori='" & IIf(RdPosteriori.Checked, "OUI", "NON") & "',DelaiEtape='" & IIf(rdDelai.Checked, txtDelaiValue.Value & " " & cmbDelaiMesure.Text, "DAO") & "' WHERE RefEtape='" & RefEtape.Text & "'"
                ExecuteNonQuery(query)
    ...
    Private Sub SupprimerEtape_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles SupprimerEtape.Click
        If (ViewEtape.RowCount > 0) Then
            ...
            query = "select * from T_PlanMarche where RefEtape='" & CodEtap & "'"
            Dim dt As DataTable = ExcecuteSelectQuery(query)
            If dt.Rows.Count > 0 Then
            ...
            Dim RepoSup As DialogResult = ConfirmMsg("Voulez-vous supprimer " & vbNewLine & "<<" & ViewEtape.GetRow(ViewEtape.FocusedRowHandle)(3).ToString & ">> ?")
            If RepoSup = DialogResult.No Then
                Exit Sub
            End If
            ...
            query = "DELETE from T_EtapeMarche where RefEtape='" & CodEtap & "'"
            ExecuteNonQuery(query)
